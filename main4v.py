...
def etc_node(state):
    prompt = (
        "당신은 조건에 맞지 않는 모든 답변을 처리하는 전문가입니다다 . "
        "응답은 반드시 다음 헤더로 시작해야 합니다:\n\n"
        "**날씨, 숙박, 식당, 교통만을 질문하세요!**\n\n"
    )
    return create_node(state, prompt)
...
def router_node(state):
    ...
    "판단해야 할 항목에 포함되지 않는 것들은 모두 'etc'로 처리하세요.\n\n"
...
def dynamic_node(state):
    ...
    for node in state.get("selected_nodes", []):
        if node == "weather":
            ...
            state = etc_node(state)
            outputs["etc"] = state["messages"][-1].content
...
# Draw the graph
try:
    graph.get_graph(xray=True).draw_mermaid_png(output_file_path="graph4v.png")
except Exception:
    pass
